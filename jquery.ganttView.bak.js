...
(function (jQuery) {
    ...
    function ganttBuild(options) {
		...
		function startBuild() {
	        ...
	        els.each(function () {
				...
				var w = jQuery("div.ganttview-vtheader", container).outerWidth() +opts.slideWidth;
                //container.css("width", (w + 2) + "px");
	            new ganttBehavior(container, opts).apply();
	            setTimeout(function(){
                    div.css({'min-height':$('.ganttview-slide-container').height()+2});
                },60)
	        });
		}
    }
	...
	var ganttChart = function(div, opts) {
        ...
        function addVtHeader(div, data, cellHeight,opts) {
            var headerDiv = jQuery("<table>", { "class": "ganttview-vtheader" }).css(opts.gridCss).attr('width','200');
            ...
            headerDiv.append(vtheader);
           var totalShowWordLength = 0;
           var headerTitle = [];
            var leftHeaderTitle=  jQuery("<tr>", { "class": "ganttview-left-header-title" }).css({
                height:opts.headerHeight,
                width:"100%"
            });

            headerDiv.append(leftHeaderTitle);
            ...
            for(var y1 =0;y1<opts.leftGridItemsShowWord.length; y1++){
                ...
                leftGridTitleTd+= "<td class='ganttview-left-header-title-div' col-num-item='"+kk+"' style='height:100%;line-height: "+opts.headerHeight+"px;width:"+(width)+"'>"+opts.leftGridItemsShowWord[y1]['text']+"</td>";
            ...
            for(var y2 =0;y2<opts.leftGridSeriesShowWord.length; y2++){
                ...
                leftGridTitleTd +="<td class='ganttview-left-header-title-div' col-num-item='"+kk+"' style='height:100%;line-height: "+opts.headerHeight+"px;width:"+(width)+"'>"+opts.leftGridSeriesShowWord[y2]['text']+"</td>"
        ...
        function addBlocks(div,opts) {
                ...
                for (var j = 0; j < opts.data[i].series.length; j++) {
                    ...
                    if(opts.dateModel){
                        ...
                        if(end_offsetQuarters>opts.headerData.length){
                            size = opts.headerData.length-offsetQuarters;
                        }
            ...
            if(min_offsetQuarters){
                //起始显示位置
                setTimeout(function(){
                    $('.ganttview-slide-container').animate({
